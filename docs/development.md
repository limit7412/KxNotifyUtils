...
- **SteamVR を起動していない状態で手動起動したとき、SteamVR が起動しないこと**（issue #12）
- SteamVR より先に手動起動した場合でも、60 秒の再試行で登録と終了の連動が働くこと
- Background で初期化しても `IVRApplications` の呼び出しと `VREvent_Quit` の受け取りが通ること
