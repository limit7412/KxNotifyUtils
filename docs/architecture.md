...
- **SteamVR が動いていないときの登録**：行えないものとして扱い、操作そのものを押せなくする。OpenVR の `IVRApplications` は vrserver が動いていないと使えず、#12 で Background 初期化へ変えてから、SteamVR を起動していない手動起動が普通の状態になった。トレイのメニューは以前から `steamvr_available` で無効にしていたが、設定ウィンドウのボタンは押せたままで、押すと黙って失敗しエラー通知が出るだけだった。両方を同じ条件で無効にする。SteamVR の設定ファイル（`appconfig.json` の `manifest_paths` と `vrappconfig/<app_key>.vrappconfig` の `autolaunch`）へ直に書けば非起動時でも登録はできるが、採らない。SteamVR は設定をメモリに持ち終了時に書き戻すため、動いている間に書いても消える（[ValveSoftware/openvr#1537](https://github.com/ValveSoftware/openvr/issues/1537)）。動いていない間に限れば書けるが、`appconfig.json` には他のアプリの登録も入っており、こちらが形式を読み違えるとそれらを巻き添えにする。文書化されていない私有の設定ファイルを書く危険に対して、得られるのは「SteamVR を起動する前に登録できる」ことだけである。
