...
module Update
  ...
  class GitHubRepository < Repository
    ...
    #
    # この上限は、積まれている数がこれより少ないリポジトリの費用を下げない（issue #30）。
    # fetch_all は満たない件数のページが返った時点で止めるため、
    # リリースが 10 件なら上限がいくつでも要求は 1 回である。
    # 効くのは上限を超えて積まれたときだけで、そこは効いてほしくない場面でもある。
    # 超えた範囲に版番号のより大きい安定版が残ると、それを見つけられなくなる。
    #
    # それでも上限は要る。最悪の場合を縛るものが他に無い。
    # 未認証の上限は IP あたり 60 回/時であり、本体は SteamVR の自動起動で立ち上がるため
    # VR を始めるたびに確認が走る。際限なくたどると、そこへ近づくほど確認自体が失敗しやすくなる。
    #
    # 10 ページとすると、最悪でも 1 回の確認で 11 要求（一覧 10 と latest 1）に収まる。
    # 1000 件を超えるまで届かず、超えても 60 回/時に対して 5 回ぶんの余地が残る。
    ...
    def fetch_releases(channel : String) : Catalog
      ...
      GitHubRepository.catalog(listed, complete) { fetch_latest_stable }
    end

    # 一覧の結果から目録を作る。latest を取りに行くかどうかもここで決める。
    #
    # 一覧を最後まで取れたなら、latest はその中に必ず含まれる。
    # /releases が返すのは下書き以外のすべてであり、latest はそのうちの 1 件だからである。
    # 取りに行っても同じタグが merge で落ちるだけで、要求を 1 つ無駄にする。
    #
    # 取りに行くのは切れていたときだけとする。
    # そのときだけは、押し出された範囲に安定版が残っていて latest から拾えることがある。
    #
    # 普段の確認が 1 要求ぶん軽くなる。MAX_PAGES を広げられるのはこの分による。
    #
    # 取りに行くかどうかを spec で確かめられるよう、latest はブロックで受け取る。
    def self.catalog(listed : Array(Release), complete : Bool, & : -> Array(Release)) : Catalog
      return Catalog.new(listed, true) if complete

      Catalog.new(merge(listed, yield), false)
    end
